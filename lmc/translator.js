...
function _translateAlias(instructions) {
  let Memory = [];
  let alias = {};
  instructions = instructions.map((v) => v.map(i => i.trim()));
  ...
  instructions.map((c_instruction) => {
    ...
    if (data) {
      if (!isNaN(Number(data)))
        instruction = Number(opcode + _correctDataLength(data.toString()));
...
let _getOPCode = (instruction) => {
  ...
  if (typeof t_instruction != "string")
    throw "Only translate from string values";
